...
parser.add_argument('--temp', type=float, default=0.9, help='Temperature for text generation')  
...
# Improved chatbot response function with robust error handling
def predict(message, history):
    start_time = time.time()
    
    try: 
        # Check if the message is a generic greeting with the improved function
        ...
        # For non-greeting messages, use RAG with increased context
        try:
            results = rag_search(mc, encoder, dict_list, message, top_k=5)
            ...
            # Improved context building with distance information
        ...
        Your answers should be comprehensive, accurate, and specific to the question asked.
        """
        USER_PROMPT = f"""
        <context>
        {context}
        ...
        Ensure your answer is thorough and directly addresses the specific question.
        ...
        try:
            # Try non-streaming first for a more robust response
            try:
                print("Attempting non-streaming response...")
                ...
                response_time = f"Response time: {time.time() - start_time:.4f} s"
                yield answer, response_time
                return
            except Exception as non_stream_error:
                print(f"Non-streaming approach failed: {str(non_stream_error)}. Trying streaming...")
            ...
            for chunk in stream:
                try:
                    ...
                    if hasattr(chunk, 'choices'):
                        ...
                        print(f"Warning: Received chunk as tuple: {chunk}")
                        content = str(chunk[0]) if chunk else None
                    else:
                        # Unknown format
                        print(f"Warning: Unknown chunk format: {type(chunk)}, {chunk}")
                        content = None
                    ...
                    print(error_msg)
                    if not partial_message:  # Only yield error if we haven't returned anything yet
                        yield f"Error while generating response: {str(chunk_error)}", response_time
                    return
            
            # If we get here but haven't yielded anything, return an error
...
def main():
    ...
    try:
        ...
        demo = ui()
        demo.queue().launch(server_name=args.host, server_port=args.port, share=True, quiet=True)
    except Exception as e:
