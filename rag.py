...
def encode_docs(docs, encoder):
    # Adjusted for better chunking
    CHUNK_SIZE = 512
    chunk_overlap = np.round(CHUNK_SIZE * 0.20, 0)  # Increased overlap for better context
    ...
    for chunk, vector in zip(chunks, embeddings):
        chunk_dict = {
            ...
            'page': chunk.metadata.get('page', 0),  # Added page number for better context
...
def search(mc, encoder, dict_list, question, top_k=5):  # Default to 5 instead of 2
    ...
    for i, res in enumerate(results[0][:2]):  # Show first 2 results only to avoid clutter
